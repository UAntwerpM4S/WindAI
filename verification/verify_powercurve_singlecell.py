...
PLOT_DIR      = Path("WindAI_power_singlecell")
...
def keep_single_farm_cells(meta: pd.DataFrame) -> pd.DataFrame:
    """Keep only farms that are the ONLY farm in their CERRA cell (drop shared cells)."""
    n_per_cell = meta.groupby(["cerra_grid_lat", "cerra_grid_lon"])["farm"].transform("size")
    single = meta[n_per_cell == 1].copy()
    print(f"  Single-farm cells: {len(single)} farms/cells "
          f"(dropped {len(meta) - len(single)} farms in shared cells)")
    print(f"    kept: {sorted(single['farm'].tolist())}")
    return single
...
def plot_metrics(
    ...
    fig1.suptitle("Belgian single-farm cells", fontsize=12)

    # --- Figure 2: Bias alone ---
    fig2, ax_bias = plt.subplots(1, 1, figsize=(7, 5))
    fig2.suptitle("Belgian single-farm cells", fontsize=12)
...
def main() -> None:
    ...
    meta             = keep_single_farm_cells(meta)   # single-farm cells only
