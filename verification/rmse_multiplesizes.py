...
FORECAST_DIRS = {
    ...
    "NoPowerGTSmall": Path("/mnt/weatherloss/WindPower/inference/CI/GTCIFINAL"),
    "NoPowerGTLarge": Path("/mnt/weatherloss/WindPower/inference/EGU/NoPowerGTLong"),
...
INIT_START = pd.Timestamp("2025-01-01 00:00:00", tz="UTC")
INIT_END   = pd.Timestamp("2025-03-31 21:00:00", tz="UTC")
LEAD_HOURS = list(range(3, 72, 3))
...
def build_spatial_index(sample_nc: Path, cerra_lat: np.ndarray, cerra_lon: np.ndarray) -> np.ndarray:
    """For each CERRA node find the index of the nearest forecast node.

    Works for both same-domain (near-identity mapping) and larger-domain
    forecasts (selects the subset of nodes overlapping the CERRA grid).
    """
    with h5py.File(str(sample_nc), "r") as f:
        fc_lat = f["latitude"][:]
        fc_lon = f["longitude"][:]
    ...
    tree = cKDTree(to_xyz(fc_lat, fc_lon))
    _, idx = tree.query(to_xyz(cerra_lat, cerra_lon))
    return idx  # shape: (n_cerra_nodes,)


def _read_one_file(args):
    """
    Worker: one process per file, uses h5py to avoid netCDF4 heap corruption.
    Returns (init_iso, {lead_hour: mean_squared_error}).
    """
    nc_path, init_iso, lead_hours, var_name, cerra_cache_items, spatial_idx = args
    ...
    try:
        with h5py.File(str(nc_path), "r") as f:
            ...
            var_all = f[var_name][:, :]  # (n_times, n_fc_cells)
        ...
        for lh in lead_hours:
            ...
            fc_vals = var_all[tidx][spatial_idx]   # subset to CERRA nodes
...
def main():
    ...
    # ── build spatial index per directory (once, from a sample file) ──────────
    spatial_indices = {}
    for label, fmap in dir_file_maps.items():
        sample_file = next(iter(fmap.values()))
        idx = build_spatial_index(sample_file, cerra_lat, cerra_lon)
        spatial_indices[label] = idx
        print(f"{label}: spatial index built ({len(idx)} CERRA nodes → forecast nodes)")
    ...
    for var in TARGET_VARS:
        ...
        for i, (label, fmap) in enumerate(dir_file_maps.items()):
            ...
            spatial_idx = spatial_indices[label]
        ...
        fig.tight_layout()
        fig.savefig(OUT_DIR / f"rmse_{var}_compare.png", dpi=150)
        plt.close(fig)
        print(f"Saved: {OUT_DIR / f'rmse_{var}_compare.png'}")
