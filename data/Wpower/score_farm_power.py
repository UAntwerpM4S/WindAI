...
Set FORECAST_DIR in the SETTINGS block below (any of it can be overridden on the command line).

Usage:
  python score_farm_power.py                       # uses the SETTINGS below
  python score_farm_power.py --forecasts DIR --region UK --plot
"""
...
# -------------------- SETTINGS --------------------
# Where farms.csv / turbines.csv / power_obs.csv live (NOT where this script lives, so the
# script can sit in verification/ while the metadata stays with the data).
WPOWER_DIR   = Path("/mnt/weatherloss/WindPower/data/WPDistr")

FORECAST_DIR = Path("/mnt/weatherloss/WindPower/inference/WPDistr/HighCapacityGT")
VAR          = "capacityfactor"          # the forecast variable ("capacityfactor" or "power")
REGION       = "Belgium"                 # Belgium | UK | all
LEAD_HOURS   = list(range(3, 37, 3))
OUT_CSV      = Path("WPDistr_scores/scores_farm_power.csv")   # relative to cwd
# --------------------------------------------------
...
def main():
    ...
    ap.add_argument("--forecasts", type=Path, default=FORECAST_DIR, help="dir of forecast_*.nc")
    ap.add_argument("--var", default=VAR, help="forecast variable (capacityfactor or power)")
    ap.add_argument("--region", default=REGION, help="Belgium | UK | all")
