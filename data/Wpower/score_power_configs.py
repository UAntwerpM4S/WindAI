...
against the ENTSO-E/Elexon observations in power_obs.csv:
  ...
  POWERCURVE  the classical baseline: the model's forecast ws100 at the farm, pushed through that
              ...
              cut-out, rated MW) and the farm's fleet in farms.csv:
                  P_curve(farm,t) = 1/2[ A_farm(ws_t) + A_farm(ws_t+3h) ]
              A_farm is rescaled so its rated plateau equals the farm's nameplate capacity, is
              applied PER CELL and capacity-weighted (SPECS_PER_CELL), and is averaged over the
              observation's 3 h window (DROP_LAST_LEAD) because power_obs is a MEAN while the
              curve is instantaneous. Both averages are of POWERS, never of winds: the curve is
              cubic on the ramp, so A(mean ws) != mean A(ws).
...
OBS_STEP_H = 3                       # power_obs at t is the MEAN over [t, t+OBS_STEP_H)

# power_obs is a 3 h MEAN, but the specs curve is an INSTANTANEOUS physical relation, so grading
# a snapshot against an average charges it an error that is bookkeeping rather than skill. The
# forecast steps at OBS_STEP_H, so the window is the adjacent pair (j, j+1) and the POWERS are
# averaged -- not the winds, because the curve is cubic on the ramp. DIRECT is trained on the
# mean already and is read as-is.
#   The last lead has no successor, so the curve is undefined there. It is then dropped from
#   EVERY method, or the methods would be scored on different samples. This assumes the lead
#   list runs out to the forecast horizon (true for the 3..36 default on a 36 h forecast).
DROP_LAST_LEAD = True
# --------------------------------------------------
...
# =============================================================================
# A farm spans several cells and the curve is nonlinear, so averaging over them does not commute
# with converting: on the cubic ramp A(mean_c ws) < mean_c A(ws), and past the knee the sign flips.
...
# Matches SPECS_PER_CELL in verification/verify_power.py and WindPowerTransformer/score_cf.py.
SPECS_PER_CELL = True
...
def main() -> None:
    ...
    # The curve cannot form its observation window at the final lead, so that lead is dropped
    # from the runs (DROP_LAST_LEAD). Drop it here too, or persistence -- and therefore the
    # skill scores that divide by it -- would be reported on a lead the others do not have.
    persist_leads = args.leads[:-1] if DROP_LAST_LEAD else args.leads
    for init in common:
        if init not in obs.index:
            continue
        pref = obs.loc[init, farms].to_numpy(float)
        for lh in persist_leads:
    ...
    for label, fmap in fmaps.items():
        ...
        for init in common:
            with xr.open_dataset(fmap[init]) as ds:
                ...
                ws_cells = ds["ws100"].values[:, cell_idx]                   # (T, C)
            ...
            if SPECS_PER_CELL:
                ...
                w = G / G.sum(1, keepdims=True)                              # (F, C)
                p_curve_all = np.column_stack([curves[f](ws_cells) @ w[i]
                                               for i, f in enumerate(farms)])
            else:
                ws_farm = farm_wind(ws_cells, G)                             # (T, F)
                p_curve_all = np.column_stack([curves[f](ws_farm[:, i])
                                               for i, f in enumerate(farms)])
            ...
            for lh in args.leads:
                ...
                # the observation window (see DROP_LAST_LEAD): average the specs curve's POWERS
                # over the adjacent pair, and require it to be the NEXT forecast step.
                nxt = t2i.get(vt + pd.Timedelta(hours=OBS_STEP_H))
                if DROP_LAST_LEAD and (nxt is None or nxt != j + 1):
                    continue
                p_curve = (0.5 * (p_curve_all[j] + p_curve_all[nxt])
                           if DROP_LAST_LEAD else p_curve_all[j])
                ptrue = obs.loc[vt, farms].to_numpy(float)
                accumulate((label, "curve"), p_curve, ptrue, k)
                if have_var:
