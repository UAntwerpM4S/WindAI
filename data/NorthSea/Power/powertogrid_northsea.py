...
"""Project offshore wind farm power/metadata onto the North Sea CERRA grids.

Adds four variables to both ``cerra_inner.zarr`` and ``cerra_outer.zarr``:
...
The script mirrors the BOZ powertogrid workflow: farms are snapped to the
closest CERRA grid point via KDTree, power time series are aligned to the
dataset time axis, and NaNs are retained when no data are present.
"""
...
# Paths
DATA_DIR = Path("/mnt/data/weatherloss/WindPower/data/NorthSea")
POWER_CSV = DATA_DIR / "Power" / "BE_UK_offshore_per_unit_3H_meanMW_shifted.csv"
META_CSV = DATA_DIR / "Power" / "windfarm_metadata.csv"
CERRA_INNER = DATA_DIR / "Cerra" / "cerra_inner.zarr"
CERRA_OUTER = DATA_DIR / "Cerra" / "cerra_outer.zarr"
...
def load_metadata() -> pd.DataFrame:
    """Load farm metadata and coerce numeric fields."""
    df = pd.read_csv(META_CSV)
    required_cols = {"farm", "lat", "lon", "capacity_mw", "turbines"}
...
def load_power_series(target_times: pd.DatetimeIndex, farms: list[str]) -> pd.DataFrame:
    ...
    # Parse to UTC, then drop timezone to match CERRA
...
def map_farms_to_grid(ds: xr.Dataset, meta_df: pd.DataFrame) -> pd.DataFrame:
    """Snap each farm to its nearest CERRA grid cell."""
    ...
    meta_df["km_error_approx"] = meta_df["deg_error"] * 111.0  # rough conversion for quick QA
    return meta_df
...
def append_power_to_zarr(zarr_path: Path):
    """Append power/mask/turbines/capacity variables to a CERRA Zarr store."""
    ...
    # Restrict to farms that appear in both mapping and power time series
...
def main():
    for store in (CERRA_INNER, CERRA_OUTER):
        append_power_to_zarr(store)
